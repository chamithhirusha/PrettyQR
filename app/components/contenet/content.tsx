...
const Content = () => {
  ...
  const [withBackground, setWithBackground] = useState(false); // checkbox state
  ...
  const generateQRCode = async () => {
    ...
    try {
      const url = await QRCode.toDataURL(input, {
        width: size,
        margin: 2,
        color: { dark: fgColor, light: withBackground ? bgColor : "#00000000" }, // use RGBA transparent if unchecked
      });
      setQrCodeUrl(url);
    } catch (err) {
  ...
  return (
    <section
      className="min-h-screen flex flex-col-reverse md:flex-row items-center justify-center gap-10 p-8
        ...
        >
          ...
          <div className="mt-4 flex flex-col gap-3">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              />
              <label htmlFor="withBackground" className="text-white/80 text-sm">
          ...
          <div className="mt-4 flex flex-col gap-2">
            <button
              ...
              className="w-full py-3 rounded-lg bg-white/10 backdrop-blur-sm border border-white/20
            ...
            <button
              onClick={resetAll}
              className="w-full py-3 rounded-lg bg-red-600/20 backdrop-blur-sm border border-red-500/30
